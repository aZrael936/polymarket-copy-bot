...
const validateRequiredEnv = (): void => {
    ...
    const required = ['USER_ADDRESSES', 'CLOB_HTTP_URL', 'CLOB_WS_URL', 'MONGO_URI'];

    // Additional vars required only for real trading
    if (!isPaperTrading) {
        required.push('PROXY_WALLET', 'PRIVATE_KEY', 'RPC_URL', 'USDC_CONTRACT_ADDRESS');
    }
...
const validateAddresses = (): void => {
    ...
    if (
        ...
        !isValidEthereumAddress(process.env.PROXY_WALLET)
    ) {
